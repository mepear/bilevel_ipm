...
class BLOCC:
    ...
    def maxminoptimizer_g(self, x, y_init, mu_init):
        mu = mu_init.copy()
        y = y_init.copy()
        for outer_iter in range (self.maxmin_g_outer_max_iters):
            ...
            for inner_iter in range (self.maxmin_g_inner_max_iters):
                ...
                if (np.linalg.norm(y - y_proj) / self.alpha_g_y) <= self.epsilon_inner_y_g:
                    ...
                    break
                print(f"Inner iter for L_g={inner_iter}, Projected Gradient norm w.r.t y of L_g={(np.linalg.norm(y - y_proj) / self.alpha_g_y)}")
                y = y_proj
    ...
    def maxminoptimizer_F(self, x, y_init, mu_init):
        mu = mu_init.copy()
        y = y_init.copy()
        for outer_iter in range (self.maxmin_F_outer_max_iters):
            ...
            for inner_iter in range (self.maxmin_g_inner_max_iters):
                ...
                if (np.linalg.norm(y - y_proj) / self.alpha_F_y) <= self.epsilon_inner_y_F:
                    ...
                    break
                print(f"Inner iter for L_F={inner_iter}, Projected Gradient norm w.r.t y of L_F={(np.linalg.norm(y - y_proj) / self.alpha_F_y)}")
                y = y_proj
    ...
    def blocc(self, x_init, y_g_init, y_F_init, mu_g_init, mu_F_init):
        x = x_init.copy()
        y_g = y_g_init.copy()
        y_F = y_F_init.copy()
        mu_g = mu_g_init.copy()
        ...
        for iter in range (self.main_max_iters):
            ...
            y_g, mu_g = self.maxminoptimizer_g(x, y_F, mu_g)
            y_F, mu_F = self.maxminoptimizer_F(x, y_g, mu_g)
