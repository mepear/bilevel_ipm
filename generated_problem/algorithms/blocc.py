...
class BLOCC:
    ...
    def blocc(self, x_init, y_g_init, y_F_init, mu_g_init, mu_F_init, max_elapsed_time, step_size_type="const"):
        ...
        y_g_temp, mu_g_temp = self.maxminoptimizer_g(x, y_F, mu_g, start_time, max_elapsed_time)
        if y_g_temp is None and mu_g_temp is None:
            print("Time limit exceeded in Lagrangian_g. Exiting blocc.")
        else:
            y_g, mu_g = y_g_temp, mu_g_temp
        y_F_temp, mu_F_temp = self.maxminoptimizer_F(x, y_g, mu_g, start_time, max_elapsed_time)
        if y_F_temp is None and mu_F_temp is None:
            print("Time limit exceeded in Lagrangian_g. Exiting blocc.")
        else:
            y_F, mu_F = y_F_temp, mu_F_temp
        ...
        for iter in range (self.main_max_iters):
            ...
            y_g_temp, mu_g_temp = self.maxminoptimizer_g(x_new, y_F, mu_g, start_time, max_elapsed_time)
            if y_g_temp is None and mu_g_temp is None:
                print("Time limit exceeded in Lagrangian_g. Exiting blocc.")
                break
            else:
            ...
            y_F_temp, mu_F_temp = self.maxminoptimizer_F(x_new, y_g, mu_g, start_time, max_elapsed_time)
            if y_F_temp is None and mu_F_temp is None:
                print("Time limit exceeded in Lagrangian_F. Exiting blocc.")
                break
            else:
                y_F, mu_F = y_F_temp, mu_F_temp
